...
// Generated 02-Apr-2022 17:22:46
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"c1_waypoint\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(c1_waypoint_M, (NULL));
        c1_waypoint_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(c1_waypoint_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(c1_waypoint_M)
                == (NULL)) || rtmGetStopRequested(c1_waypoint_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(c1_waypoint_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
