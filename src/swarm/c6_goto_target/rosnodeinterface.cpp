...
// Generated 17-Jun-2022 10:09:30
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"c6_goto_target\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(c6_goto_target_M, (NULL));
        c6_goto_target_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(c6_goto_target_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(c6_goto_target_M)
                == (NULL)) || rtmGetStopRequested(c6_goto_target_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(c6_goto_target_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
