...
// Generated 17-Jun-2022 14:53:20
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"c6_formation\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(c6_formation_M, (NULL));
        c6_formation_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(c6_formation_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(c6_formation_M)
                == (NULL)) || rtmGetStopRequested(c6_formation_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(c6_formation_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
