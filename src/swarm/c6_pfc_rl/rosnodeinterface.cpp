...
// Generated 17-Jun-2022 10:37:02
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"c6_pfc_rl\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(c6_pfc_rl_M, (NULL));
        c6_pfc_rl_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(c6_pfc_rl_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(c6_pfc_rl_M)
                == (NULL)) || rtmGetStopRequested(c6_pfc_rl_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(c6_pfc_rl_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
